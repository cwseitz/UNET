...
from arwn.data._constants import _SETUP_ARGS_KEY
from arwn.data.fields import (
...
from arwn.dataloaders import (
    AnnDataLoader,
    SemiSupervisedDataLoader,
    SemiSupervisedDataSplitter,
)
from arwn.model._utils import _init_library_size
from arwn.module import SCANVAE
from arwn.train import SemiSupervisedTrainingPlan, TrainRunner
from arwn.train._callbacks import SubSampleLabels
from arwn.utils import setup_anndata_dsp

from ._arwn import arwn
...
class SCANVI(RNASeqMixin, VAEMixin, ArchesMixin, BaseModelClass):
    ...
    ----------
    adata
        AnnData object that has been registered via :meth:`~arwn.model.SCANVI.setup_anndata`.
    n_hidden
    ...
    **model_kwargs
        Keyword args for :class:`~arwn.module.SCANVAE`

    Examples
    --------
    >>> adata = anndata.read_h5ad(path_to_anndata)
    >>> arwn.model.SCANVI.setup_anndata(adata, batch_key="batch", labels_key="labels")
    >>> vae = arwn.model.SCANVI(adata, "Unknown")
    >>> vae.train()
    ...
    2. :doc:`/tutorials/notebooks/scarches_arwn_tools`
    ...
    def from_arwn_model(
        ...
        **scanvi_kwargs,
        ...
        """
        Initialize scanVI model with weights from pretrained :class:`~arwn.model.arwn` model.

        Parameters
        ----------
        arwn_model
            Pretrained arwn model
        labels_key
            key in `adata.obs` for label information. Label categories can not be different if
            labels_key was used to setup the arwn model. If None, uses the `labels_key` used to
            setup the arwn model. If that was None, and error is raised.
        unlabeled_category
            Value used for unlabeled cells in `labels_key` used to setup AnnData with arwn.
        adata
            AnnData object that has been registered via :meth:`~arwn.model.SCANVI.setup_anndata`.
        scanvi_kwargs
            kwargs for scANVI model
        """
        arwn_model._check_if_trained(
            message="Passed in arwn model hasn't been trained yet."
        )

        scanvi_kwargs = dict(scanvi_kwargs)
        init_params = arwn_model.init_params_
        ...
        for k, v in {**non_kwargs, **kwargs}.items():
            if k in scanvi_kwargs.keys():
                warnings.warn(
                    ...
                    + "pretrained arwn model with value {}.".format(v)
                )
                del scanvi_kwargs[k]

        if adata is None:
            adata = arwn_model.adata
        else:
            # validate new anndata against old model
            arwn_model._validate_anndata(adata)

        arwn_setup_args = deepcopy(arwn_model.adata_manager.registry[_SETUP_ARGS_KEY])
        arwn_labels_key = arwn_setup_args["labels_key"]
        if labels_key is None and arwn_labels_key is None:
            raise ValueError(
                "A `labels_key` is necessary as the arwn model was initialized without one."
            )
        if arwn_labels_key is None:
            arwn_setup_args.update(dict(labels_key=labels_key))
        cls.setup_anndata(
            adata,
        ...
        arwn_state_dict = arwn_model.module.state_dict()
        scanvi_model.module.load_state_dict(arwn_state_dict, strict=False)
    ...
    def train(
        ...
        **trainer_kwargs,
        ...
        plan_kwargs
            Keyword args for :class:`~arwn.train.SemiSupervisedTrainingPlan`. Keyword arguments passed to
