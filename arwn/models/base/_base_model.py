...
from arwn import settings
from arwn.data import AnnDataManager
from arwn.data._compat import manager_from_setup_dict
from arwn.data._constants import _MODEL_NAME_KEY, _arwn_UUID_KEY, _SETUP_ARGS_KEY
from arwn.data._utils import _assign_adata_uuid
from arwn.dataloaders import AnnDataLoader
from arwn.model._utils import parse_use_gpu_arg
from arwn.module.base import PyroBaseModuleClass
...
class BaseModelMetaClass(ABCMeta):
    """
    Metaclass for :class:`~arwn.model.base.BaseModelClass`.

    Constructs model class-specific mappings for :class:`~arwn.data.AnnDataManager` instances.
    ``cls._setup_adata_manager_store`` maps from AnnData object UUIDs to :class:`~arwn.data.AnnDataManager` instances.
    This mapping is populated everytime ``cls.setup_anndata()`` is called.
    ``cls._per_isntance_manager_store`` maps from model instance UUIDs to AnnData UUID::class:`~arwn.data.AnnDataManager` mappings.
    These :class:`~arwn.data.AnnDataManager` instances are tied to a single model instance and populated either
...
class BaseModelClass(metaclass=BaseModelMetaClass):
    """Abstract class for arwn-tools models."""
    ...
    def register_manager(cls, adata_manager: AnnDataManager):
        """
        Registers an :class:`~arwn.data.AnnDataManager` instance with this model class.

        Stores the :class:`~arwn.data.AnnDataManager` reference in a class-specific manager store.
        Intended for use in the ``setup_anndata()`` class method followed up by retrieval of the
        :class:`~arwn.data.AnnDataManager` via the ``_get_most_recent_anndata_manager()`` method in
        the model init method.

        Notes
        -----
        Subsequent calls to this method with an :class:`~arwn.data.AnnDataManager` instance referring to the same
        underlying AnnData object will overwrite the reference to previous :class:`~arwn.data.AnnDataManager`.
        """
    ...
    def _register_manager_for_instance(self, adata_manager: AnnDataManager):
        """
        Registers an :class:`~arwn.data.AnnDataManager` instance with this model instance.

        Creates a model-instance specific mapping in ``cls._per_instance_manager_store`` for this
        :class:`~arwn.data.AnnDataManager` instance.
        """
    ...
    def _get_most_recent_anndata_manager(
        ...
        """
        Retrieves the :class:`~arwn.data.AnnDataManager` for a given AnnData object specific to this model class.

        Checks for the most recent :class:`~arwn.data.AnnDataManager` created for the given AnnData object via
        ``setup_anndata()`` on model initialization. Unlike :meth:`arwn.model.base.BaseModelClass.get_anndata_manager`,
        ...
        """
        if _arwn_UUID_KEY not in adata.uns:
            if required:
                raise ValueError(
        ...
        adata_id = adata.uns[_arwn_UUID_KEY]
    ...
    def get_anndata_manager(
        ...
        """
        Retrieves the :class:`~arwn.data.AnnDataManager` for a given AnnData object specific to this model instance.

        Requires ``self.id`` has been set. Checks for an :class:`~arwn.data.AnnDataManager`
    ...
    def _validate_anndata(
        ...
        if adata_manager is None:
            logger.info(
                "Input AnnData not setup with arwn-tools. "
    ...
    def load(
        ...
        adata
            ...
            as AnnData is validated against the saved `arwn` setup dictionary.
        ...
        if "arwn_setup_dict_" in attr_dict:
            arwn_setup_dict = attr_dict.pop("arwn_setup_dict_")
    ...
    def setup_anndata(
        ...
        **kwargs,
        ...
        the implementation must call :meth:`~arwn.model.base.BaseModelClass.register_manager`
        on a model-specific instance of :class:`~arwn.data.AnnDataManager`.
        """
    ...
    def view_anndata_setup(
        ...
        adata
            ...
            :meth:`~arwn.data.AnnDataManager.transfer_fields`.
