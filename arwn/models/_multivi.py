...
from arwn._types import Number
from arwn._utils import _doc_params
...
from arwn.dataloaders import DataSplitter
from arwn.model._utils import (
    _get_batch_code_from_category,
    scatac_raw_counts_properties,
    scrna_raw_counts_properties,
)
from arwn.model.base import UnsupervisedTrainingMixin
from arwn.module import MULTIVAE
from arwn.train import AdversarialTrainingPlan, TrainRunner
from arwn.train._callbacks import SaveBestState
from arwn.utils._docstrings import doc_differential_expression, setup_anndata_dsp
...
class MULTIVI(VAEMixin, UnsupervisedTrainingMixin, BaseModelClass):
    ...
    ----------
    adata
        AnnData object that has been registered via :meth:`~arwn.model.MULTIVI.setup_anndata`.
    n_genes
    ...
    **model_kwargs
        Keyword args for :class:`~arwn.module.PEAKVAE`

    Examples
    --------
    >>> adata_rna = anndata.read_h5ad(path_to_rna_anndata)
    >>> adata_atac = arwn.data.read_10x_atac(path_to_atac_anndata)
    >>> adata_multi = arwn.data.read_10x_multiome(path_to_multiomic_anndata)
    >>> adata_mvi = arwn.data.organize_multiome_anndatas(adata_multi, adata_rna, adata_atac)
    >>> arwn.model.MULTIVI.setup_anndata(adata_mvi, batch_key="modality")
    >>> vae = arwn.model.MULTIVI(adata_mvi)
    >>> vae.train()
    ...
    * The main batch annotation, specified in ``setup_anndata``, should correspond to
    ...
    def train(
        ...
        **kwargs,
        ...
        plan_kwargs
            Keyword args for :class:`~arwn.train.TrainingPlan`. Keyword arguments passed to
    ...
    def get_accessibility_estimates(
        ...
        ----------
        adata
            AnnData object that has been registered with arwn. If `None`, defaults to the
            AnnData object used to initialize the model.
        indices
            Indices of cells in adata to use. If `None`, all cells are used.
        n_samples_overall
    ...
    def get_normalized_expression(
        ...
        This is denoted as :math:`\rho_n` in the arwn paper.
