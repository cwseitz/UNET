...
import arwn
from arwn._types import AnnDataField
from arwn.utils import attrdict
...
class AnnDataManager:
    """
    Provides an interface to validate and process an AnnData object for use in arwn-tools.
    ...
    via :meth:`~arwn.data.AnnDataManager.register_fields`. This decouples the generalized
    definition of the arwn-tools interface with the registration of an instance of data.
    """
    ...
    def __init__(
        ...
        self._registry = {
            _constants._arwn_VERSION_KEY: arwn.__version__,
    ...
    def _validate_anndata_object(adata: AnnData):
        """For a given AnnData object, runs general arwn-tools compatibility checks."""
        if adata.is_view:
            raise ValueError("Please run `adata = adata.copy()`")

    def _get_setup_method_args(self) -> dict:
        """
        Returns the ``setup_anndata`` method arguments used to initialize this :class:`~arwn.data.AnnDataManager` instance.

        Returns the ``setup_anndata`` method arguments, including the model name,
        that were used to initialize this :class:`~arwn.data.AnnDataManager` instance
    ...
    def _assign_uuid(self):
        ...
        arwn_uuid = self.adata.uns[_constants._arwn_UUID_KEY]
        self._registry[_constants._arwn_UUID_KEY] = arwn_uuid
    ...
    def register_fields(
        ...
        source_registry
            Registry created after registering an AnnData using an :class:`~arwn.data.AnnDataManager` object.
        transfer_kwargs
    ...
    def transfer_fields(self, adata_target: AnnData, **kwargs) -> AnnDataManager:
        ...
        Creates a new :class:`~arwn.data.AnnDataManager` instance with the same set of fields.
    ...
    def adata_uuid(self) -> str:
        ...
        return self._registry[_constants._arwn_UUID_KEY]
    ...
    def _view_data_registry(self) -> rich.table.Table:
        ...
        for registry_key, data_loc in self.data_registry.items():
            ...
            if attr_key is None:
                arwn_data_str = f"adata.{attr_name}"
            else:
                arwn_data_str = f"adata.{attr_name}['{attr_key}']"
            t.add_row(registry_key, arwn_data_str)
    ...
    def view_registry(self, hide_state_registries: bool = False) -> None:
        ...
        """
        version = self._registry[_constants._arwn_VERSION_KEY]
        rich.print(f"Anndata setup with arwn-tools version {version}.")
        rich.print()
