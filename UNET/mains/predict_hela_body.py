...
def main(config,model_path,stack_path,prefix):
    ch0_stack = prefix + '_mxtiled_corrected_ch0.tif'
    ch1_stack = prefix + '_mxtiled_corrected_ch1.tif'
    ...
    model.eval()
    ch0 = tifffile.imread(ch0_stack).astype(np.int16)
    ch1 = tifffile.imread(ch1_stack).astype(np.int16)
    nt,nx,ny = ch0.shape
